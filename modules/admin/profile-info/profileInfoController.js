...
ProfileInfoCtr.deleteProfileInfo = async (req, res) => {
    try {
        const deleteProfileInfo = await profileInfoModel.deleteOne({ _id: req.params.id });
...
ProfileInfoCtr.list = async (req, res) => {
    try {
        ...
        if (getList) {
            await client.set("profile-info-list", JSON.stringify(getList), "EX", 60 * 10);
            return res.status(200).json({
...
ProfileInfoCtr.getProfileInfoDetails = async (req, res) => {
    try {
        const getProfileInfoDetails = await profileInfoModel.findOne({ _id: req.params.id });
...
ProfileInfoCtr.listForAdmin = async (req, res) => {
    try {
        ...
        const list = await profileInfoModel.find({})
            .sort({ createdAt: -1 })
